...
| Seat selection | Not available | `GetSeatMap` endpoint |
...
```
`cellCategory` observed value: `"Seat"` (`UNKNOWN` other categories — e.g. galley/lavatory/exit-row markers likely exist per standard seat-map conventions but were not present in the one observed example). `cellDetails` is a **stringified object** (observed literally as a PowerShell/.NET `ToString()`-style dump: `"@{cellID=...; cellCode=1A; cellAvailability=Occupied; seatPriceDetails=; cellCharacteristics=System.Object[]}"` in the extracted example) — `INFERRED` **this stringification is an artifact of this document's own PowerShell-based extraction process, not necessarily the live API's actual JSON shape** — the real API almost certainly returns `cellDetails` as a proper nested JSON object (`{cellID, cellCode, cellAvailability, seatPriceDetails, cellCharacteristics}`), and this should be **confirmed directly against a live response** before writing structural assertions — flagged as `UNKNOWN` pending that confirmation, not asserted as fact.

Observed `cellAvailability` values: `Occupied`, `AvailablePaid` — `INFERRED` other values likely include an "available free" and "blocked/unavailable" state; `UNKNOWN` full enumeration.

**Response Mapping**: no collection variables are set from this response (`INFERRED` from the extract — GetSeatMap has no test script variable assignments at all) — the seat **code** (e.g., `cellCode: "1A"`) the tester selects must be manually carried into Book/Hold's `PassengersSelectedAncillaries...AncillaryCode` for the `Seat` category, analogous to ancillary codes from GetAncillary.

**IDs Generated**: none captured automatically.

**Common Errors**: requesting for an offer with `isSeatMapRequired: false`; segment not in the confirmed offer.

**Testing Notes**: Since no variable is auto-captured, **manual test execution must copy the chosen `cellCode` by hand** into the next request — this is a natural point for tester error in manual E2E testing; call this out explicitly in test case steps (§7).
...
**Required Request Fields** (V2) — **markedly different**: the one saved V2 Book request body in the collection carries `ResponseId`, `selectedOfferId`, `supplier`, `credentialsSelector`, and `PassengersSelectedAncillaries` — **no `passengersList` at all**, consistent with §1.10's AddPassenger-replaces-inline-passengers rule. `INFERRED` **caveat**: this specific saved V2 Book example's variable references (`{{ANCResponseID}}`, `{{ANCofferId}}`) show it was captured mid-way through an **ancillary-flow** test run (reusing GetAncillary's output variables) — it is a real, working example of "Book after AddPassenger+GetAncillary," not necessarily the *minimal* Book payload for a plain no-enrichment V2 flow (flows 25–30). For a plain V2 flow with no Bundle/Ancillary/SeatMap, `INFERRED` the minimal required fields are `supplier`, `credentialsSelector`, `ResponseId` (= AddPassenger's `getAddPaxResponceID`), `selectedOfferId` (= AddPassenger's `getAddPaxOfferId`) — `UNKNOWN`, not directly evidenced by a saved example, flag as needing confirmation before writing the flow-25 positive test.

**`PassengersSelectedAncillaries.<paxRef>` shape** (V2, when ancillaries are part of the flow): `SetSegmentsAncillaries: {<segmentRefId or compound-key>: [{AdditionalServiceCategory, AncillaryCode, Quantity}]}`, optionally also `SetJourneysAncillaries` (journey-level rather than segment-level ancillaries, e.g. a through-fare baggage allowance spanning multiple segments — observed in the same example).
...
```
{
  ...
  contacts,
  selectedOffer: { offerId, offerJourneys, passengerFareBreakdown, priceDetails, refundability, isDealCodeApplied, appliedDealCode, isPromoted, appliedPromotionCode },
...
**IDs Required By Next Endpoint**: `fareConfirmResponseId`+`selectedOfferId` (V1) or `getAddPaxResponceID`+`getAddPaxOfferId` (V2, `UNKNOWN`-confirm) as input; produces `pnr`/`bookingToken`/`gdsPNR` consumed by Retrieve and Void.
...
```
{
  ...
  passengersSelectedAncillaries, journeys, flightSegments, priceClasses, baggageDetails,
...
| `cellCategory` | `Seat` (only value observed) | §2.7 |
| `cellAvailability` | `Occupied`, `AvailablePaid` (observed); others `UNKNOWN` | §2.7 |
...
4. Full endpoint-level positive+negative+business-rule tests (§8.1 items 1,2,5) for all 15 endpoints, both versions.
